...
'''
Tensorflow implementation of PNN
Reference:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run PNN.")
    ...
    parser.add_argument('--batch_size', type=int, default=4096,
                        help='Batch size.')
    parser.add_argument('--use_inner', type=bool, default=True,
                        help='product type, inner=True or outer=False.')
    parser.add_argument('--D1', type=int, default=5,
                        help='the signal dimension')
...
class PNN(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size, embedding_size, pretrain_flag, save_file,
                 ...
                 batch_norm_decay, keep, use_inner, D1, l2_reg= 0.0, loss_type="mse", deep_layers=[32, 32],  verbose=True, random_seed=2018,
    ...
    def _initialize_weights(self):
        ...
        ## product layers
        if self.use_inner:
            all_weights["product_quadratic_inner"] = tf.Variable(tf.random_normal([self.D1, self.field_size], 0.0, 0.01))
        else:
